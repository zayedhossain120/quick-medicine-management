...
export async function POST(request: NextRequest) {
  try {
    const profitData = await request.json();
    const { memberId, amount, method, note, date } = profitData;

    if (!memberId || !amount || !method) {
      return NextResponse.json(
        { error: "Member ID, amount, and method are required" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        message: "Profit share added successfully",
      },
      { status: 201 }
    );
  } catch (error) {
